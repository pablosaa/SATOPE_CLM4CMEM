...
USE YOMCMEMFIELDS, ONLY : N, JJ, CLNAME, fTVL, fTVH,  fs_laiL, ftfrac, fs_laiL &
                    ...
                    & , ftskin,fwc_lsm,fsand,fclay,fwater, mask, ftheta_inc ! PSG: insert ftheta_inc
...
! PSG: -------------------------------------------------------------
! PSG: testing to read gradient
! PSG: -------------------------------------------------------------
CLNAME='../forcing/theta_inc.nc' ! PSG:
CCVARNC_NAME='SLOPE' ! PSG:
CALL io_cmemnetcdf(CLNAME,LD_firstcall,LD_infocoord) ! PSG:
JJ = 1 ! PSG:
DO JTIME = 1, NTIMES_SM ! PSG:
DO JJ2D = 1,NLONS*NLATS ! PSG:
  ftheta_inc(JJ,1) =  fncfield(JJ2D,1) ! PSG:
  JJ = JJ+1 ! PSG:
ENDDO ! PSG:
ENDDO ! PSG:
! PSG: end of gradient testing ----------------------------------
