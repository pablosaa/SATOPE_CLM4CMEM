...
!---------------------------------------------------------------------------------------
print*,'just to end define time...',xlats(1:5)
...
print*, 'ftb_toa size...', shape(ftb_toa)
print*,'after define time1...',shape(tbh_out)
print*,'after define time2...',shape(tbv_out)
IF(status /= nf90_noerr) CALL ABOR1(trim(nf90_strerror(status)))
print*,'after define time3...'
!
...
!-------------------------
!
   CLNAME='../output/out_level2_'//CNAMEID//'_'//cfreq//'_'//cangle//'.nc'
   status = NF90_CREATE(CLNAME,NF90_CLOBBER,ncid)
   IF(status /= nf90_noerr) CALL ABOR1(trim(nf90_strerror(status)))
!
!
...
!-------------------------
!
   CLNAME='../output/out_level3_'//CNAMEID//'_'//cfreq//'_'//cangle//'.nc'
   status = NF90_CREATE(CLNAME,NF90_CLOBBER,ncid)
   IF(status /= nf90_noerr) CALL ABOR1(trim(nf90_strerror(status)))
!
!
