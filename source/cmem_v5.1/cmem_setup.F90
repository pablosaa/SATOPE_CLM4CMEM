...
INTEGER(KIND=JPIM) :: i
!!type(SATELLITE) :: SAT  ! PSG: to include the SATELLITE type
...
! Open namelist: 
if(len_trim(INPUTNAMLST).lt.1) INPUTNAMLST='input'   ! PSG: default input
OPEN(NULNAM,FILE='../inputlst/'//INPUTNAMLST,STATUS='OLD')  ! PSG: default was 'input' 
...
CIDIEL = 'Dobson'  ! PSG: 'Wang' back to previous version
CITEFF = 'Wigneron'
CISMR = 'Fresnel'
CIRGHR = 'Wsimple' 
CIVEG = 'Wigneron'
...
IF ( nlay_soil_ls .ne. nlay_soil_ls_default ) Then
 ...
 !   
  ...
  OPEN(NULTMP,FILE='../forcing/LSM_VERTICAL_RESOL.asc',status='old') ! netcdfcase
...
IF(CFINOUT/= 'gribapi'.AND.CFINOUT/= 'netcdf'.AND.CFINOUT/= 'ascii'.and.CFINOUT/= 'clm')&
& CALL ABOR1('Wrong CFINOUT choice.  Choose gribapi, netcdf or ascii')
!-------------------------------------
...
!-------------------------------------
 ...
 lamcm = lam * 100._JPRM 
 kcm = k / 100._JPRM 
 !! costheta = cos(theta*pi/180._JPRM)  ! PSG: defined later in main()
 !! sintheta = sin(theta*pi/180._JPRM)  ! PSG: defined later in main()
...
IF ( LOFIELDEXP .AND. CFINOUT /= 'ascii') CALL ABOR1('Stop: ascii format required for field experiments')
